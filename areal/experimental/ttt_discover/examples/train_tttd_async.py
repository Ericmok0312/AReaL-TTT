...
class TTTDPPOTrainer(PPOTrainer):
    ...
    def __init__(self, config: TTTDPPOConfig):
        ...
        logger.info(
            f"[SAMPLER_CONFIG] max_head_offpolicyness={max_head_offpolicyness}, "
    ...
    def _clear_stale_state_after_recovery(self):
        ...
        is_dp_head = (
            self.actor.rank == 0
        )  # Global rank 0 is the unique head for logging/saving
        ...
        if self.recover_info:
            ...
            if dispatcher is not None:
                ...
                with sm.lock:
                    ...
                    if is_dp_head:
                        logger.info(
                            f"[Resume] staleness_capacity = ({max_staleness} + {version} + 1) * {consumer_bs} - {sm.rollout_stat.accepted} = {capacity}"
                        )
    ...
    def _init_training_history_logger(self):
        ...
        if is_dp_head:
            existing_snapshots = (
                len(self.history_logger.history) if self.recover_info else 0
            )
    ...
    def train(
        ...
        # Keep a reference so close() can call workflow.shutdown() and clean up
        # background resources (e.g. AsyncRewardWrapper's ProcessPoolExecutor,
        # ALE-Bench sessions, visualization servers).
        ...
        if config.sampler.lazy_puct_sampling and hasattr(
            self.sampler, "save_version_snapshot"
        ):
            self.sampler.save_version_snapshot(0)
            logger.info(
                "[INIT] Saved initial PUCT snapshot version=0 for step 0 sampling"
            )
        ...
        logger.info(
            f"[DEBUG] max_steps={max_steps}, start_step={start_step}, range={start_step}-{max_steps - 1}"
        )
        logger.info(
            f"[DEBUG] config.max_steps={getattr(config, 'max_steps', None)}, config.total_train_epochs={config.total_train_epochs}"
        )
        ...
        for global_step in range(start_step, max_steps):
            loop_count += 1
            logger.info(
                f"[DEBUG] Loop #{loop_count}: global_step={global_step}, max_steps={max_steps}"
            )
            ...
            if max_steps_limit is not None and global_step >= max_steps_limit:
                logger.info(
                    f"[DEBUG] Breaking loop at global_step={global_step}, max_steps_limit={max_steps_limit}"
                )
                break
            ...
            if use_scheme_1 and hasattr(workflow, "set_current_batch_tracking"):
                workflow.set_current_batch_tracking(
                    step=global_step, batch_size=batch_size, n_samples=group_size
                )
            ...
            if dist.is_initialized():
                local_rollout_tokens = torch.tensor(
                    [rollout_total_tokens], dtype=torch.long, device=self.actor.device
                )
            ...
            if use_scheme_1 and hasattr(workflow, "wait_for_batch_completion"):
                logger.info(
                    f"[SCHEME1][Step {global_step}] Waiting for batch completion..."
                )
                scheme_1_wait_time = workflow.wait_for_batch_completion(
                    batch_step=global_step
                )
                logger.info(
                    f"[SCHEME1][Step {global_step}] Waited {scheme_1_wait_time:.2f}s"
                )
            ...
            if use_scheme_1 and hasattr(workflow, "get_current_batch_parent_ids"):
                ...
                puct_update_result = workflow.get_pending_updates(
                    clear=True, current_step=global_step, parent_ids=batch_parent_ids
                )
                workflow.clear_current_batch_tracking()
            else:
                # Scheme 2: Use all available data (default)
                puct_update_result = workflow.get_pending_updates(
                    clear=True, current_step=global_step
                )
            ...
            if len(puct_update_result) == 5:
                ...
                local_children, local_parents, local_failed, rollout_metadata = (
                    puct_update_result
                )
                cross_batch_info = {}
            ...
            if (
                cross_batch_info.get("mode") == "strict"
                and cross_batch_info.get("n_delayed", 0) > 0
            ):
                logger.info(
                    f"[Step {global_step}] STRICT_PUCT: Prevented cross-batch contamination. "
            ...
            if rollout_metadata:
                ...
                exec_times = [
                    m.get("exec_time_ms", 0)
                    for m in rollout_metadata
                    if m.get("exec_time_ms", 0) > 0
                ]
                ...
                if is_dp_head:
                    logger.info(
                        f"[Step {global_step}] STALENESS_BATCH: n={len(staleness_values)}, "
                        f"avg={avg_staleness:.2f}, min={min_staleness}, max={max_staleness}"
                    )
                    if exec_times:
                        avg_exec_time = sum(exec_times) / len(exec_times)
                        logger.info(
                            f"[Step {global_step}] EXEC_TIME: n={len(exec_times)}, avg={avg_exec_time:.2f}ms"
                        )
            ...
            if async_overhead != 0:
                logger.info(
                    f"[Async Monitor][Step {global_step}][Rank {self.actor.dp_rank}] "
            ...
            unique_parents = (
                set(p.id for p in local_parents) if local_parents else set()
            )
            n_unique_parents = len(unique_parents)
            avg_children_per_parent = (
                len(local_children) / n_unique_parents if n_unique_parents > 0 else 0
            )

            logger.info(
                f"[Step {global_step}][Rank {self.actor.dp_rank}] Local updates: "
            ...
            if local_parents and local_children:
                ...
                for pid, count in sorted(parent_child_counts.items())[
                    :10
                ]:  # Limit to first 10 to avoid log spam
                    logger.info(
                        f"[PARENT_CHILD_MAP] step={global_step} rank={self.actor.dp_rank} "
                ...
                for parent in local_parents:
                    ...
                    if pid in self.sampler._m:
                        ...
                        try:
                            ...
                            logger.debug(
                                f"[PUCT_TRACK] Failed to log update for {pid[:8]}: {e}"
                            )
            ...
            if dist.is_initialized() and self.actor.data_parallel_world_size > 1:
                ...
                if hasattr(self.sampler, "_states"):
                    ...
                    state_checksum = (
                        int(hashlib.md5(hash_input).hexdigest(), 16) & 0xFFFFFFFF
                    )
                else:
                    state_checksum = 0
                ...
                dist.all_gather_object(
                    all_checksums, state_checksum, group=self.actor.data_parallel_group
                )

                if len(set(all_checksums)) != 1:
                    logger.warning(
                        f"[Step {global_step}] Cross-rank sampler state INCONSISTENT!"
                    )
            ...
            if dist.is_initialized():
                local_actor_tokens = torch.tensor(
                    [actor_update_tokens], dtype=torch.long, device=self.actor.device
                )
            ...
            if hasattr(workflow, "get_exec_time_stats"):
                ...
                if exec_time_stats:
                    ...
                    logger.info(
                        f"[EXEC_TIME_STATS] step={global_step} "
            ...
            logger.info(
                f"[Rank {self.actor.dp_rank}][Step {global_step}] Rollouts: {local_rollouts} (global: {global_rollouts}), "
            ...
            if config.actor.should_compute_prox_logp():
                ...
                logger.info(
                    f"[Rank {self.actor.dp_rank}][Step {global_step}] should_compute_prox_logp=False, prox_logp not computed"
                )
            ...
            if "prox_logp" in rollout_batch and rollout_batch["prox_logp"] is not None:
                ...
                logger.info(
                    f"[Rank {self.actor.dp_rank}][Step {global_step}] Before ppo_update: prox_logp shape={prox_logp.shape}, numel={prox_logp.numel()}, expected={bs * max_seqlen}"
                )
            else:
                logger.warning(
                    f"[Rank {self.actor.dp_rank}][Step {global_step}] Before ppo_update: prox_logp is None or missing!"
                )
            ...
            metrics.update(
                {
                    ...
                    "train/importance_weight": stats.get(
                        "ppo_actor/update/importance_weight/avg", 0.0
                    ),
                    "train/clip_ratio": stats.get(
                        "ppo_actor/update/clip_ratio/avg", 0.0
                    ),
            ...
            logger.info(
                f"[DEBUG] Loop #{loop_count} (global_step={global_step}) completed successfully"
            )

        # DEBUG: Loop ended
        logger.info(
            f"[DEBUG] Loop ended after {loop_count} iterations. global_step={global_step if 'global_step' in locals() else 'N/A'}"
        )
        ...
        if history_path:
            logger.info(
                f"[TTTLogger][Rank {self.actor.dp_rank}] Final training history saved to {history_path}"
            )
            ...
            # Determine a reasonable benchmark value for visualization.
            benchmark_value = 2.635983  # AC1 default
            if hasattr(self, "_workflow_kwargs") and self._workflow_kwargs:
                env = self._workflow_kwargs.get("env")
                if env is not None:
                    benchmark_value = getattr(env, "target_score", benchmark_value)
        ...
        # === Save Full Sampler Checkpoint ===
        # Persist the complete PUCT state, including full version snapshots, so that
        # the sampler can be resumed or evaluated without approximation. This is
        # especially important for ALE-Bench, where each problem's sampler must be
        # kept isolated and fully reproducible.
        if hasattr(self.sampler, "save_full"):
            try:
                full_checkpoint_path = self.sampler.save_full(
                    step=getattr(
                        self.config, "max_steps", self.config.total_train_epochs
                    )
                    - 1,
                )
                logger.info(
                    f"[SAMPLER][Rank {self.actor.dp_rank}] Full sampler checkpoint saved to "
                    f"{full_checkpoint_path}"
                )
            except Exception as e:
                logger.warning(
                    f"[SAMPLER][Rank {self.actor.dp_rank}] Failed to save full sampler checkpoint: {e}"
                )
    ...
    def close(self):
        ...
        Follows the AReaL pattern: do TTT-Discover specific cleanup first, then
        delegate to ``PPOTrainer.close()`` for the standard teardown (saver,
        datasets, data controller, stats logger, engines, perf tracer).
        ``FSDPEngine.destroy()`` handles ``dist.destroy_process_group()`` when
        the engine owns the global group, so we do not repeat it here.
        """
        # 1. ALE-Bench keeps long-lived Docker sessions/containers. Close it
        #    first so any worker processes blocked inside env.execute() can
        #    return before we shut down the ProcessPoolExecutor below.
        if hasattr(self, "_workflow_kwargs") and self._workflow_kwargs:
            env = self._workflow_kwargs.get("env")
                ...
                for method_name in ("close", "stop", "shutdown"):
                    if hasattr(session, method_name):
                        try:
                            ...
                            logger.info(
                                f"[TTTDPPOTrainer] ALE-Bench session {method_name}() called."
                            )
                        except Exception as e:
                            logger.warning(
                                f"[TTTDPPOTrainer] Error closing ALE-Bench session: {e}"
                            )
                        break

        # 2. Shutdown the TTT-Discover workflow. This releases the
        #    AsyncRewardWrapper's ProcessPoolExecutor workers.
        if hasattr(self, "workflow") and self.workflow is not None:
            try:
                self.workflow.shutdown()
                logger.info("[TTTDPPOTrainer] Workflow shutdown completed.")
            except Exception as e:
                logger.warning(
                    f"[TTTDPPOTrainer] Error during workflow.shutdown(): {e}"
                )
            self.workflow = None

        # 3. Standard AReaL cleanup. TTT-Discover doesn't use eval_rollout,
        #    but the parent close() checks for None before destroying it.
        super().close()
...
def main(args):
    ...
    # ============================================================
    # ALE-Bench-specific path setup.
    # When env_type='ale_bench', isolate checkpoints per problem_id by
    # suffixing the trial name. This lets the same config train each
    # ALE-Bench lite problem independently with a clean sampler.
    # ============================================================
    problem_id = getattr(config.sampler, "problem_id", "")
    if getattr(config.sampler, "env_type", "") == "ale_bench" and problem_id:
        # Make sure problem_id appears in the trial name so outputs are isolated.
        if problem_id not in config.trial_name:
            config.trial_name = f"{config.trial_name}_{problem_id}"
        if problem_id not in config.experiment_name:
            config.experiment_name = f"{config.experiment_name}_{problem_id}"

        # Propagate the new names to all nested configs that already resolved
        # the YAML interpolation at load time.
        for nested in [
            config.actor,
            getattr(config, "ref", None),
            getattr(config, "critic", None),
            getattr(config, "rollout", None),
            getattr(config, "vllm", None),
            getattr(config, "saver", None),
            getattr(config, "recover", None),
            getattr(config, "evaluator", None),
            getattr(config, "stats_logger", None),
            getattr(config, "perf_tracer", None),
        ]:
            if nested is None:
                continue
            if hasattr(nested, "experiment_name"):
                nested.experiment_name = config.experiment_name
            if hasattr(nested, "trial_name"):
                nested.trial_name = config.trial_name
            if (
                hasattr(nested, "fileroot")
                and getattr(nested, "fileroot", None) == config.cluster.fileroot
            ):
                # Keep fileroot unchanged; paths are derived from experiment/trial names.
                pass

        # Update the sampler checkpoint dir to be problem-specific.
        config.sampler.checkpoint_dir = os.path.join(
        ...
        logger.info(
            f"[ALE-Bench] Isolated checkpoint paths for problem_id={problem_id}: "
            f"experiment={config.experiment_name}, trial={config.trial_name}, "
            f"sampler_checkpoint_dir={config.sampler.checkpoint_dir}"
        )
    ...
    logger.info(
        f"[DEBUG] config.sampler.lazy_puct_sampling = {config.sampler.lazy_puct_sampling}"
    )
