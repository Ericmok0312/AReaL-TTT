...
class TTTDiscoverWorkflowV2(RolloutWorkflow):
    ...
    def __init__(
        ...
        | None = None,  # Per-rank slot-level. Default: batch_size * group_size
        ...
        self.hint_placement = (
            hint_placement  # 'append', 'replace_code', or 'replace_no_code'
        )
        ...
        if strict_sync_mode:
            if batch_size is None or group_size is None:
                raise ValueError(
                    "batch_size and group_size must be provided when strict_sync_mode=True"
                )
            self.expected_rollouts_per_step = batch_size * group_size
            logger.info(
                f"TTTDiscoverWorkflowV2: Strict sync mode enabled, expected_rollouts_per_step={self.expected_rollouts_per_step}"
            )
        ...
        if lazy_sampling:
            ...
            if vllm_concurrency is None:
                ...
                logger.info(
                    f"Auto vllm_concurrency = batch_size({self.batch_size}) * "
            ...
            logger.info(
                f"TTTDiscoverWorkflowV2: Lazy sampling enabled, "
        ...
        if puct_update_mode not in ["eager", "strict"]:
            raise ValueError(
                f"puct_update_mode must be 'eager' or 'strict', got {puct_update_mode}"
            )
        ...
        if puct_update_mode == "strict":
            logger.info(
                "TTTDiscoverWorkflowV2: STRICT PUCT update mode enabled. "
                "Cross-batch contamination will be prevented."
            )
        ...
        if max_reward_workers is None:
            max_reward_workers = int(
                os.environ.get("TTTD_MAX_CODE_WORKERS", "64")
            )  # Increased from 16
        ...
        wrapper_timeout = (
            eval_timeout * 10 + 600
        )  # 10x + 10min buffer for queue wait (very generous)
    ...
    def _create_trajectory(
        ...
        if breakthrough_child is not None:
            ...
            improvement = (
                breakthrough_child.value if breakthrough_child.value is not None else 0
            ...
            if cache_key not in self._batch_parents:
                ...
                if getattr(sampler, "sampling_strategy", None) == "breakthrough_parent":
                    ...
                    for p, c in zip(my_parents, my_children):
                        if c is not None:
                            improvement = (c.value if c.value is not None else 0) - (
                                p.value if p.value is not None else 0
                            )
                            pair_info.append(
                                f"{p.id[:8]}->{c.id[:8]}(+{improvement:.4f})"
                            )
                        else:
    ...
    def get_version_mapping_info(self) -> dict:
        ...
        return {
            "enabled": True,
            "batch_mappings": dict(self.sampler._batch_version_mappings)
            if self.sampler
            else {},
            "sampler_mappings": dict(self.sampler._version_mapping)
            if self.sampler
            else {},
            "available_snapshots": sorted(self.sampler._version_snapshots.keys())
            if self.sampler
            else [],
        }
    ...
    def validate_version_mapping(self, batch_id: int, expected_target: int) -> bool:
        ...
        if actual is None:
            logger.warning(
                f"[VERSION_VALIDATION] batch_id={batch_id} not found in sampler mappings"
            )
            return False
    ...
    async def _execute_rollout(
        ...
        sampled_step = data.get(
            "_sampled_step",
            self._current_version if hasattr(self, "_current_version") else 0,
        )
        ...
        try:
            ...
            async with self._pending_lock:
                ...
                if episode_key not in self._parent_episodes:
                    ...
                    logger.debug(
                        f"[PUCT_TRACK] Started new parent episode for {state.id[:8]}... "
                ...
                if staleness_key not in self._staleness_tracker:
                    ...
                    logger.debug(
                        f"[STALENESS_INIT] parent_id={state.id} sampled_step={sampled_step} "
                ...
                if (
                    hasattr(self, "_current_batch_parent_ids")
                    and sampled_step == self._current_batch_step
                ):
                    self._current_batch_parent_ids.add(state.id)
                    logger.debug(
                        f"[SCHEME1_BATCH][Step {sampled_step}] REGISTER | "
                        f"parent={state.id[:8]}... "
        ...
        try:
            ...
            req = ModelRequest(
                ...
                gconfig=self.gconfig.new(
                    n_samples=1, max_new_tokens=max_new_tokens_phase1
                ),
                tokenizer=self.tokenizer,
            )
            ...
            if not result.is_valid:
                logger.warning(
                    f"Validation failed: reward={reward:.4f}, fail_type={result.fail_type}"
                )
                ...
                try:
                    ...
                    async with self._pending_lock:
                        ...
                        try:
                            ...
                            current_version = (
                                -1
                            )  # Fallback if engine doesn't support versioning

                        # Use composite key (parent_id, sampled_step) to handle same parent in multiple steps
                        staleness_key = (pid, sampled_step)

                        # DEBUG: Log all available keys in staleness_tracker
                        available_keys = list(self._staleness_tracker.keys())[
                            :5
                        ]  # Limit to first 5
                        logger.debug(
                            f"[STALENESS_DEBUG] Looking for key=({pid[:8]}..., {sampled_step}), "
                        ...
                        if staleness_key not in self._staleness_tracker:
                            ...
                            parent_keys = [
                                (p, s)
                                for p, s in self._staleness_tracker.keys()
                                if p == pid
                            ]
                            if parent_keys:
                                logger.warning(
                                    f"[STALENESS_FALLBACK] parent_id={pid} sampled_step={sampled_step} not initialized, "
                            ...
                            self._staleness_tracker[staleness_key] = {
                                ...
                                "children_completed": 0,
                                "total_expected": self.gconfig.n_samples
                                if hasattr(self.gconfig, "n_samples")
                                else None,
                                "parent_timestep": state.timestep,
                                "exec_times_ms": [],
                            }
                        ...
                        logger.info(
                            f"[STALENESS] parent_id={pid} sampled_step={sampled_step_at_init} "
                        ...
                        self._rollout_metadata.append(
                            {
                                ...
                                "parent_exec_time_ms": getattr(
                                    state, "exec_time_ms", None
                                ),
                                "child_id": child.id,
                ...
                try:
                    async with self._pending_lock:
                        ...
                        if staleness_key in self._staleness_tracker:
                            ...
                            tracker["exec_times_ms"].append(exec_time_ms)
                            logger.debug(
                                f"[STALENESS_FAIL] parent_id={state.id} sampled_step={sampled_step} "
                        ...
                        sampled_step_at_init = tracker_data.get(
    ...
    def set_current_batch_tracking(self, step: int, batch_size: int, n_samples: int):
        ...
        if (
            hasattr(self, "_current_batch_parent_ids")
            and len(self._current_batch_parent_ids) > 0
        ):
            logger.warning(
                f"[SCHEME1_BATCH][Step {step}] PREVIOUS_BATCH_NOT_CLEARED | "
        ...
        logger.info(
            f"[SCHEME1_BATCH][Step {step}] START | "
    ...
    def wait_for_batch_completion(
        self, batch_step: int, poll_interval: float = 0.1
    ) -> float:
        ...
        logger.info(
            f"[SCHEME1_WAIT][Step {batch_step}] START | "
        ...
        while True:
            ...
            if current_time - last_log_time >= 5.0:
                ...
                logger.info(
                    f"[SCHEME1_WAIT][Step {batch_step}] PROGRESS | "
                ...
                if incomplete_parents:
                    ...
                    logger.debug(
                        f"[SCHEME1_WAIT][Step {batch_step}] INCOMPLETE_SAMPLE | "
            ...
            if all_complete:
                elapsed = time.time() - start_time
                logger.info(
                    f"[SCHEME1_WAIT][Step {batch_step}] SUCCESS | "
    ...
    def get_pending_updates(
        ...
        if self.puct_update_mode == "strict" and current_step is not None:
            ...
            cross_batch_info["n_delayed"] = len(
                cross_batch_info["delayed_children_ids"]
            )

            # Log cross-batch prevention
            if cross_batch_info["n_delayed"] > 0:
                logger.info(
                    f"[STRICT_PUCT] Step {current_step}: "
            ...
            if current_step is not None:
                ...
                if cross_batch_info["n_delayed"] > 0:
                    logger.debug(
                        f"[STREAMING_PUCT] Step {current_step}: "
        ...
        if parent_ids is not None:
            logger.info(
                f"[SCHEME1_FILTER][Step {current_step}] BEFORE | "
            ...
            if len(parent_ids) == 0:
                logger.warning(
                    "[SCHEME1_FILTER] Empty parent_ids filter! No parents will be returned."
                )
            ...
            scheme1_metadata = [
                m for m in filtered_metadata if m.get("parent_id") in parent_ids
            ]
            ...
            expected_children = (
                len(parent_ids) * self._expected_n_samples
                if self._expected_n_samples
                else n_children
            )
            ...
            total_completed = n_children + len(
                scheme1_failed
            )  # Total finished (success + failed)
            ...
            for pid in parent_ids:
                ...
                if not tracker:
                    ...
                    completed = tracker.get("children_completed", 0)
                    expected = tracker.get(
                        "total_expected", self._expected_n_samples or 1
                    )
                    if completed < expected:
            ...
            logger.info(
                f"[SCHEME1_FILTER][Step {current_step}] AFTER | "
        ...
        if self._staleness_tracker and all_children:
            ...
            for (pid, sampled_step), tracker in self._staleness_tracker.items():
                ...
                total_expected = (
                    tracker.get("total_expected") or tracker["children_completed"]
                )
                ...
                if not is_complete:
                    ...
                    logger.info(
                        f"[STALENESS_INCOMPLETE] parent_id={pid} sampled_step={sampled_step} "
            ...
            if staleness_list:
                ...
                logger.info(
                    f"[STALENESS_BATCH] n_parents={len(staleness_list)} "
        ...
        if clear:
            ...
            if "parents_to_cleanup" in locals():
                for key in parents_to_cleanup:
                    if key in self._staleness_tracker:
                        pid, step = key
                        logger.debug(
                            f"[STALENESS_CLEANUP] Removing complete parent {pid} step={step} from tracker"
                        )
            ...
            logger.debug(
                f"[WORKFLOW] Incremented current step: {old_step} -> {self._current_step} "
        ...
        parents_to_record = (
            filtered_parents if self.puct_update_mode == "strict" else all_parents
        )
        children_to_record = (
            filtered_children if self.puct_update_mode == "strict" else all_children
        )
        ...
        for parent in parents_to_record:
            ...
            if episode:
                ...
                n_children_in_update = len(
                    [
                        c
                        for c in children_to_record
                ...
                if n_children_in_update > 0:  # Only record if actually updating
                    ...
                    if expected and total_children >= expected:
                        ...
                        logger.debug(
                            f"[PUCT_TRACK] Episode {episode['episode_id']} for parent {pid[:8]}... "
    ...
    def start_parent_episode(
        ...
        existing_episodes = [
            ep for ep in self._parent_episodes.values() if ep["parent_id"] == parent_id
        ]
        ...
        logger.debug(
            f"[PUCT_TRACK] Started episode {episode_id} for parent {parent_id[:8]}... "
    ...
    def _get_active_episode(self, parent_id: str) -> dict | None:
        ...
        episodes = [
            ep for key, ep in self._parent_episodes.items() if key[0] == parent_id
        ]
    ...
    def shutdown(self):
        ...
        AsyncRewardWrapper uses ProcessPoolExecutor with automatic cleanup via
        weakref. We clear the reference and force a garbage collection so the
        finalizer runs deterministically at shutdown time.
        """
        if hasattr(self, "async_reward_fn") and self.async_reward_fn is not None:
            logger.info("Cleaning up AsyncRewardWrapper...")
            # Drop the wrapper so weakref.finalize can run.
