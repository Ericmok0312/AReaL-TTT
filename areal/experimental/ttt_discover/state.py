...
class AleBenchState(State):
    ...
    code: str  # the code that generated the result
    raw_score: float | None  # average raw score per public case
    parent_raw_scores: list[float]  # ancestor average raw scores (most recent first)
    ...
    def to_prompt(
        ...
        """Generate prompt value context using the average raw score.

        Overrides the base class to display ``raw_score`` (the average ALE-Bench
        raw score over all public cases) instead of ``value`` (which is kept
        ...
        if current_raw is not None:
            # raw_score is the average score per public case, already in the
            ...
            # is better.
            value_ctx += (
                f"\nCurrent {metric_name} (average raw score per public case) "
            ...
            if target is not None:
                ...
                if prev_raw is not None:
                    value_ctx += (
                        f"\nPrevious average {metric_name}: {prev_raw:.6f}. "
